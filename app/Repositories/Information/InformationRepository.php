    ...
    public function getUserContribute()
    {
        return $this->model
            ...
            ->selectRaw('users.first_name
                ...
                ,SUM(contributes.status=' . Information::PENDING . ')  AS pending
                ,SUM(contributes.status=' . Information::PUBLISHED . ') AS published
                ,SUM(contributes.status=' . Information::FUTURE . ') AS future')
            ->groupBy('owner_id')
            ->orderBy('totals', 'desc')
            ->paginate(10);
    }
